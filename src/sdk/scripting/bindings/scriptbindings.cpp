...
namespace ScriptBindings
{
    ...
    {
        int paramCount = sq_gettop(vm);
        if (paramCount == 3)
        {
            wxString *key = Sqrat::ClassType<wxString>::GetInstance(vm, 2);
            if(key == NULL)
                return sq_throwerror(vm, _SC("have no key"));
            ...
            {
                wxString val = *Sqrat::ClassType<wxString>::GetInstance(vm, 3);
        ...
        else if (paramCount == 4)
        {
            ...
            {
                Sqrat::Var<bool> value(vm,4);
                Manager::Get()->GetConfigManager(_T("scripts"))->Write(*key, *val, value.value);
                return SC_RETURN_OK;
            }
        }
        return sq_throwerror(vm,"Invalid arguments to \"ConfigManager::Write\"");
    }
